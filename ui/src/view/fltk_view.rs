...
use crate::view::chessview::{ChessViewModel, Msg, SquareType};
...
impl GTKView {
    ...
    fn draw_button_at(&self, i: i8, j: i8, button: &mut Button) {
        ...
        if let Some(name) = self.chess_view.get_image_name_at(i, j) {
            let path = format!("ui/images/{name}");
            let img = SvgImage::load(path).unwrap();
            button.set_image(Some(img));
        } else {
            let path = "ui/images/transparent_square.svg".to_string();
            let img = SvgImage::load(path).unwrap();
            button.set_image(Some(img));
        }
    ...
    pub fn run_app(&mut self) {
        ...
        while app.wait() {
            if let Some(msg) = r.recv() {
                // Call the chess view to run the logic
